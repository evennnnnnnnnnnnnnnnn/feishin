...
Feishin wraps the Mantine components and hooks it uses in `packages/ui/src/components/` and `packages/ui/src/hooks/`. Feature code imports those wrappers (`@feishin/ui/...`), not `@mantine/*`.

## Default (most UI work)

1. Import from `@feishin/ui/components/...` and `@feishin/ui/hooks/...`.
...
- **Debugging a wrapper** — behavior/bug lives in `packages/ui/src/components/*` or `packages/ui/src/hooks/*` (or adjacent shared Mantine adapters), or
- **Adding a new wrapped** component or hook — introducing a new `@feishin/ui/...` re-export that did not exist.
