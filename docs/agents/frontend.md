...
**Mantine:** feature UI imports wrappers under `@feishin/ui/components` and `@feishin/ui/hooks`, not `@mantine/core`. Open `docs/agents/mantine.md` only when debugging those wrappers or adding a new one.
...
1. **Shared wrapper** — `@feishin/ui/components/...` (design-system primitive).
...
Layout primitives: `Box`, `Flex`, `Stack`, `Group`, `Grid` from `@feishin/ui/components/...`. Prefer them over one-off layout divs when composing screens.
...
Global CSS is rare (`packages/ui/src/styles/global.css` and third-party/theme overrides) — don’t add feature styles there.
